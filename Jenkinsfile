        ...
        stage('AWS') {
            ...
            container('awscli-kubectl-helm') {
                stage('push EKS') {
                    ...
                    ecrLogin()
                        def accountIdentity = awsIdentity()
                        def IMAGE_REPO_NAME = "${accountIdentity.account}.dkr.ecr.${AWS_DEFAULT_REGION}.amazonaws.com/demo"
                        ...
                        def helmOptions = "--set-string \"image.repository=104568846065.dkr.ecr.ap-southeast-1.amazonaws.com/demo:40,image.tag=${BUILD_NUMBER}\" --atomic --cleanup-on-fail"
